...
const roleContent = {
  patient: {
    ...
    description: 'Create a patient account and land directly in the patient dashboard.',
  },
  doctor: {
    accent: 'bg-emerald-400',
    title: 'Doctor Registration',
    description: 'Create a doctor account and land directly in the doctor dashboard.',
  },
};
        ...
        <div className="mx-auto w-full max-w-[440px] rounded-[1.8rem] border border-white/12 bg-[#0d224b]/92 p-7 shadow-[0_35px_90px_rgba(2,13,36,0.28)] backdrop-blur-xl fade-in-up">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              ...
              className="w-full rounded-3xl bg-gradient-to-r from-emerald-500 to-teal-400 px-5 py-3 text-sm font-semibold text-slate-950 shadow-[0_15px_40px_rgba(16,185,129,0.2)] transition hover:from-emerald-400 hover:to-teal-300 disabled:cursor-not-allowed disabled:opacity-60"
            >
              {isSubmitting ? 'Registering...' : `Create ${role} account`}
            </button>
          </form>

          <div className="mt-6 border-t border-white/10 pt-4 text-sm text-slate-400">
            Already registered?{' '}
            <Link href={`/login?role=${role}`} className="text-sky-300 hover:text-white">
              Sign in as {role}
            </Link>
          </div>
        </div>
