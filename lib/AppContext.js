...
import { ensureDoctorProfile, fetchRegisteredDoctors, readCachedDoctors } from './doctors';
import {
    ...
    const syncAppState = async () => {
      ...
      try {
        ...
        if (doctorsError) {
          ...
          const mergedDoctors =
            doctorsData.length > 0 ? doctorsData : getCachedDoctorDirectory(nextDoctor || fallbackDoctor);
